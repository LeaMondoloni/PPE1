...
while read -r URL; do
	...
	encodage=$(curl -Ls $URL -D - -o "./aspirations/fich-$lineno.html" | grep -Eo "charset=(\w|-)+" | cut -d= -f2)
...
#fin du tableau
echo "<tr><td>$lineno</td><td>$code</td><td>$encodage</td><td><a href=\"$URL\">$URL</a></td><td><a href="././aspirations/fich-$lineno.html">html</a></td><td><a href="././dumps-text/fich-$lineno.txt">txt</a></td><td>$NB_OCC</td><td><a href="././contextes/fich-$lineno.txt">contextes</a></td><td><a href="././concordances/fich-$lineno.html">concordance</a></td></tr>" >> $fichier_tableau 
	echo -e "\t--------------------------------"
	lineno=$((lineno+1));
done < $fichier_urls
